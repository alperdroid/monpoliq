// ── Sentiment Analysis Edge Function v2.3 ──
// ALL regex literals converted to new RegExp() constructors to fix Deno edge-runtime boot crash
...
function extractMainContent(html:string):string {
  // ALL regex converted to new RegExp() constructors
  const patterns = [
...
function scoreSentiment(text:string, title='') {
  ...
  const wc = Math.max(combined.split(new RegExp('\\s+')).length, 1);
...
async function parseBoardRss(feedUrl:string, sourceLabel:string, key:string, cutoffStr:string, limit:number, fetchText:boolean):Promise<SentimentItem[]> {
  ...
  const textUrls:string[] = [], statItems:{ri:typeof rssItems[0];pub:string}[] = [], commItems:{ri:typeof rssItems[0];pub:string}[] = [];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (bank === 'both' || bank === 'FED') {
      ...
      result.fed = { items: deduped, ...scores };
    ...
    if (bank === 'both' || bank === 'ECB') {
      ...
      const scores = computeDualScores(deduped);
      result.ecb = { items: deduped, ...scores };
