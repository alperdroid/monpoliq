...
// ── Scorer ──
function scoreSentiment(text:string, title:string=''):{hawk_pts:number;dove_pts:number;net_score:number;label:string;word_count:number;reasons:string[]} {
...
function extractText(html:string):string {
  ...
  return t.replace(/\s+/g,' ').trim().slice(0,60000);
}

async function fetchPageText(url:string):Promise<string> {
  try {
    const r=await fetch(url,{headers:{'User-Agent':'Mozilla/5.0 (compatible; CentralBankSentiment/2.3)'}});
    if (!r.ok) return '';
...
// ── RSS parsing ──
function parseXmlItems(xml:string):{title:string;link:string;pubDate:string;category:string}[] {
  const items:{title:string;link:string;pubDate:string;category:string}[]=[];
  const re=/<item>([\s\S]*?)<\/item>/gi;
  let m;
  while ((m=re.exec(xml))!==null) {
    const b=m[1];
    const title=(b.match(/<title>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/title>/s)||[])[1]?.trim()||'';
    const link=(b.match(/<link>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/link>/s)||[])[1]?.trim()||'';
    const pubDate=(b.match(/<pubDate>(.*?)<\/pubDate>/s)||[])[1]?.trim()||'';
    const category=(b.match(/<category>(.*?)<\/category>/s)||[])[1]?.trim().toLowerCase()||'';
...
function parseAtomEntries(xml:string):{title:string;link:string;updated:string;summary:string}[] {
  ...
  while ((m=re.exec(xml))!==null) {
    const b=m[1];
    const title=(b.match(/<title[^>]*>(.*?)<\/title>/s)||[])[1]?.trim()||'';
    const lm=b.match(/<link[^>]+rel="alternate"[^>]+href="([^"]+)"/);
    const link=lm?lm[1]:'';
    const updated=(b.match(/<updated>(.*?)<\/updated>/s)||[])[1]?.trim()||'';
    const summary=(b.match(/<summary[^>]*>(.*?)<\/summary>/s)||[])[1]?.trim()||'';
...
function parseRssDate(text:string):string|null {
  try { const d=new Date(text); if (isNaN(d.getTime())) return null; return d.toISOString().split('T')[0]; }
...
async function analyzeStatRelease(title:string,url:string,summary:string,rules:StatRule[]):{net_score:number;label:string;metric:string;value_found:number|null;weight:number;method:string} {
  ...
  if (rule&&rule.number_regex&&rule.direction!=='text_only') {
    ...
    if (url) { const pt=await fetchPageText(url); if (pt) { const nm2=pt.match(new RegExp(rule.number_regex,'i')); if (nm2) { const v=parseFloat(nm2[1]); if (!isNaN(v)) { const r=scoreFromValue(v,rule.hawkish_threshold!,rule.dovish_threshold!,rule.direction,rule.weight,rule.metric); return {...r,method:'numeric'}; } } } }
  }
...
async function fetchFredData(apiKey:string,daysCutoff:number=90):Promise<SentimentItem[]> {
  ...
  for (const spec of FRED_POLICY_SERIES) {
    try {
      ...
      const resp=await fetch(url); if (!resp.ok) { await resp.text(); continue; }
      const data=await resp.json();
      const obs=(data.observations||[]).filter((o:any)=>o.value!=='.');
      if (!obs.length) continue;
      if (new Date(obs[0].date)<cutoff) continue;
      ...
      items.push({bank:'FED',source:'FRED',item_date:obs[0].date,title:`${spec.metric}: ${val.toFixed(2)} (${spec.series_id})`,url:`https://fred.stlouisfed.org/series/${spec.series_id}`,is_statistical:true,hawk_pts:0,dove_pts:0,net_score:result.net_score,label:result.label,word_count:0,reasons:['fred_api'],stat_metric:result.metric,stat_value:result.value_found,stat_weight:result.weight});
    } catch(e) { console.error(`FRED error [${spec.series_id}]:`,e); }
  }
  return items;
}
...
async function fetchEcbData(daysBack:number=60,fetchText:boolean=true):Promise<SentimentItem[]> {
  ...
  for (const feed of feeds) {
    try {
      const resp=await fetch(feed.url,{headers:{'User-Agent':'Mozilla/5.0 (compatible; CentralBankSentiment/2.3)'}});
      if (!resp.ok) { await resp.text(); continue; }
      const xml=await resp.text();
      const rssItems=parseXmlItems(xml);
      for (const ri of rssItems.slice(0,20)) {
        const pub=parseRssDate(ri.pubDate); if (!pub||pub<cutoffStr) continue;
        if (feed.key==='stats') {
          ...
          let text=''; if (fetchText&&ri.link) text=await fetchPageText(ri.link);
    ...
    } catch(e) { console.error(`ECB feed error [${feed.label}]:`,e); }
  }
  // Eurostat Atom
  try {
    ...
    const resp=await fetch(euroUrl,{headers:{'User-Agent':'Mozilla/5.0 (compatible; CentralBankSentiment/2.3)'}});
    if (resp.ok) {
      const xml=await resp.text();
...
// ── Board RSS parser (speeches, testimony, press) ──
async function parseBoardRss(feedUrl:string,sourceLabel:string,key:string,cutoffStr:string,limit:number,fetchText:boolean):Promise<SentimentItem[]> {
  const items:SentimentItem[]=[];
  try {
    const resp=await fetch(feedUrl,{headers:{'User-Agent':'Mozilla/5.0 (compatible; CentralBankSentiment/2.3)'}});
    if (!resp.ok) { await resp.text(); return items; }
...
const FOMC_CAL=FOMC_BASE+'/monetarypolicy/fomccalendars.htm';

async function fetchFomcCalendar():Promise<string> {
  try {
    const r=await fetch(FOMC_CAL,{headers:{'User-Agent':'Mozilla/5.0 (compatible; CentralBankSentiment/2.3)'}});
    if (!r.ok) { await r.text(); return ''; }
    return await r.text();
  } catch(_e) { return ''; }
}

async function fetchFomcStatements(cutoffStr:string,limit:number=3):Promise<SentimentItem[]> {
  const items:SentimentItem[]=[];
  try {
    const html=await fetchFomcCalendar(); if (!html) return items;
    const re=/href="([^"]*newsevents\/pressreleases\/monetary\d{8}[^"]*)"/gi;
...
async function fetchFomcMinutes(cutoffStr:string,limit:number=2):Promise<SentimentItem[]> {
  const items:SentimentItem[]=[];
  try {
    const html=await fetchFomcCalendar(); if (!html) return items;
    const re=/href="([^"]*fomcminutes\d{8}[^"]*)"/gi;
    const links:{date:string;url:string}[]=[];
    let m;
    while ((m=re.exec(html))!==null) {
      const href=m[1].startsWith('http')?m[1]:FOMC_BASE+m[1];
...
async function fetchFomcPressConferences(cutoffStr:string,limit:number=2):Promise<SentimentItem[]> {
  const items:SentimentItem[]=[];
  try {
    const html=await fetchFomcCalendar(); if (!html) return items;
    const re=/href="([^"]*fomcpresconf\d{8}[^"]*)"/gi;
    const links:{date:string;url:string}[]=[];
    let m;
    while ((m=re.exec(html))!==null) {
      const href=m[1].startsWith('http')?m[1]:FOMC_BASE+m[1];
...
async function parseRegionalFed(cutoffStr:string,fetchText:boolean):Promise<SentimentItem[]> {
  ...
  for (const [fedName,feedUrl] of Object.entries(REGIONAL_FED_FEEDS)) {
    ...
    } catch(_e) { /* skip unavailable feeds */ }
  }
  return items;
}

// ── Main Fed orchestrator ──
async function fetchFedData(daysBack:number=60,fetchText:boolean=true):Promise<SentimentItem[]> {
  const cutoff=new Date(); cutoff.setDate(cutoff.getDate()-daysBack);
  const cutoffStr=cutoff.toISOString().split('T')[0];
  console.log('Fed: FOMC statements...');
  const stmts=await fetchFomcStatements(cutoffStr,3);
  console.log('Fed: '+stmts.length+' statements');
  console.log('Fed: FOMC minutes...');
  const mins=await fetchFomcMinutes(cutoffStr,2);
  console.log('Fed: '+mins.length+' minutes');
  console.log('Fed: FOMC press conferences...');
  const pc=await fetchFomcPressConferences(cutoffStr,2);
  console.log('Fed: '+pc.length+' press conferences');
  console.log('Fed: Board speeches...');
  const speeches=await parseBoardRss('https://www.federalreserve.gov/feeds/speeches.xml','Fed Speech','speeches',cutoffStr,20,fetchText);
  console.log('Fed: '+speeches.length+' speeches');
  console.log('Fed: Board testimony...');
  const testimony=await parseBoardRss('https://www.federalreserve.gov/feeds/testimony.xml','Fed Testimony','testimony',cutoffStr,15,fetchText);
  console.log('Fed: '+testimony.length+' testimony');
  console.log('Fed: Press releases...');
  const press=await parseBoardRss('https://www.federalreserve.gov/feeds/press_all.xml','Fed Press','press',cutoffStr,20,fetchText);
  console.log('Fed: '+press.length+' press releases');
  console.log('Fed: Regional Fed speeches...');
  const regional=await parseRegionalFed(cutoffStr,fetchText);
  console.log('Fed: '+regional.length+' regional speeches');
  return [...stmts,...mins,...pc,...speeches,...testimony,...press,...regional];
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (bank==='both'||bank==='FED') {
      ...
      if (FRED_KEY) { const fredItems=await fetchFredData(FRED_KEY,daysBack+30); fedItems.push(...fredItems); }
      ...
      try {
        ...
        if (deduped.length>0) await supabase.from('sentiment_items').insert(deduped.map(i=>({bank:i.bank,source:i.source,item_date:i.item_date,title:i.title,url:i.url,is_statistical:i.is_statistical,hawk_pts:i.hawk_pts,dove_pts:i.dove_pts,net_score:i.net_score,label:i.label,word_count:i.word_count,reasons:i.reasons,stat_metric:i.stat_metric,stat_value:i.stat_value,stat_weight:i.stat_weight})));
        await supabase.from('sentiment_scores').insert({bank:'FED',score_1_avg:fedScores.score_1.avg,score_1_count:fedScores.score_1.n,score_1_label:fedScores.score_1.sentiment,score_1_dist:fedScores.score_1.dist,score_2_avg:fedScores.score_2.avg,score_2_count:fedScores.score_2.n,score_2_label:fedScores.score_2.sentiment,score_2_dist:fedScores.score_2.dist});
    ...
    if (bank==='both'||bank==='ECB') {
      ...
      try {
        ...
        if (deduped.length>0) await supabase.from('sentiment_items').insert(deduped.map(i=>({bank:i.bank,source:i.source,item_date:i.item_date,title:i.title,url:i.url,is_statistical:i.is_statistical,hawk_pts:i.hawk_pts,dove_pts:i.dove_pts,net_score:i.net_score,label:i.label,word_count:i.word_count,reasons:i.reasons,stat_metric:i.stat_metric,stat_value:i.stat_value,stat_weight:i.stat_weight})));
        await supabase.from('sentiment_scores').insert({bank:'ECB',score_1_avg:ecbScores.score_1.avg,score_1_count:ecbScores.score_1.n,score_1_label:ecbScores.score_1.sentiment,score_1_dist:ecbScores.score_1.dist,score_2_avg:ecbScores.score_2.avg,score_2_count:ecbScores.score_2.n,score_2_label:ecbScores.score_2.sentiment,score_2_dist:ecbScores.score_2.dist});
